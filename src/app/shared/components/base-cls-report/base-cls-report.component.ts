...
export abstract class BaseClsReportComponent {
  ...
  protected abstract getReportData(
    fromDate: string,
    toDate: string
  ): Observable<ClsStat[]>;
    ...
    this.widgetData = [
      {
        ...
        icon: 'fas fa-stethoscope',
        title: 'Tổng Lượt Khám',
        value: NumberUtils.format(totalExam),
        caption: 'Thực hiện khám',
      ...
      },
      {
        ...
        icon: 'fas fa-procedures',
        title: 'Tổng Nhập Viện',
        value: NumberUtils.format(totalAdmission),
        caption: 'Số ca nhập viện',
      ...
      },
      {
        ...
        icon: 'fas fa-door-open',
        title: topRoomName,
        value: NumberUtils.format(topRoomValue),
  ...
  protected buildCharts(
    ...
    const dateLabels = sortedDates.map(
      d => this.datePipe.transform(new Date(d), 'dd/MM') || d
    );
      ...
      series: [
        {
          ...
          label: {
            ...
            color: this.palette.textPrimary,
            formatter: (params: any) =>
              `${params.name}: ${NumberUtils.format(params.value)} (${params.percent}%)`,
          },
