...
import { ACTION_FOOTER_SHORTCUTS, getShortcutDisplayString } from '../../../core/config/keyboard-shortcuts.config';
    ...
    this.actions$.pipe(
      ...
      switchMap((actions: FooterAction[] | null) => {
        if (!actions || actions.length === 0) {
          return new Observable<{ action: FooterAction, event: KeyboardEvent }>(observer => observer.complete());
        }

        const shortcutObservables: Observable<{ action: FooterAction, event: KeyboardEvent }>[] = [];

        // 1. Register explicit shortcuts from actions
        actions.forEach(action => {
          if (action.shortcut && !action.disabled) {
            const sub = this.shortcutService.listen(action.shortcut, true).pipe(
              map((e) => ({ action, event: e.event }))
            );
        ...
        const primary = actions.find(a => a.className?.includes('btn-primary')) || actions[actions.length - 1];

        if (primary && !primary.disabled && !primary.shortcut) {
          const defaultEnter = this.shortcutService.listen(ACTION_FOOTER_SHORTCUTS.PRIMARY_ENTER, true).pipe(
            map((e) => ({ action: primary, event: e.event }))
          );
          const defaultSave = this.shortcutService.listen(ACTION_FOOTER_SHORTCUTS.PRIMARY_SAVE, true).pipe(
            map((e) => ({ action: primary, event: e.event }))
          );
