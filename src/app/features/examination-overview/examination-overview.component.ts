...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, WidgetCardComponent, ChartCardComponent, ReusableTableComponent],
  providers: [DatePipe],
...
export class ExaminationOverviewComponent implements OnInit {
  ...
  public setRange(range: 'today' | 'thisWeek' | 'thisMonth' | 'thisQuarter' | 'thisYear'): void {
    ...
    if (range === 'today') { /* defaults to now */ }
  ...
  public loadData(): void {
    ...
    this.reportService.getExaminationOverview(this.fromDate, this.toDate)
      .pipe(finalize(() => { this.isLoading = false; this.isInitialLoad = false; this.cd.markForCheck(); }))
      .subscribe({
        next: (data) => {
          this.rawData = data.map(i => ({...i, NGAY_TIEP_NHAN: DateUtils.formatToDisplay(i.NGAY_TIEP_NHAN)}));
          this.calculateWidgets(data);
          this.buildCharts(data);
        },
        error: () => this.toastService.showError('Không thể tải dữ liệu báo cáo.')
      });
  }
  ...
  private calculateWidgets(data: ExaminationStat[]): void {
    ...
    this.widgetData = [
      { id: 'total', icon: 'fas fa-users', title: 'Tổng Tiếp Nhận', value: fmt(t.total), caption: 'Total', accentColor: this.palette.deepSapphire },
      { id: 'ck', icon: 'fas fa-stethoscope', title: 'Khám Bệnh (CK)', value: fmt(t.ck), caption: 'Clinic', accentColor: this.palette.primary },
      { id: 'emergency', icon: 'fas fa-ambulance', title: 'Cấp Cứu', value: fmt(t.cc), caption: 'Emergency', accentColor: this.palette.pastelCoral },
      { id: 'inpatient', icon: 'fas fa-procedures', title: 'Nội Trú', value: fmt(t.nt), caption: 'Inpatient', accentColor: this.palette.warning },
      { id: 'daycare', icon: 'fas fa-clinic-medical', title: 'ĐT Ngoại Trú', value: fmt(t.dnt), caption: 'Daycares', accentColor: this.palette.tealMidtone },
    ];
  }
  ...
  private buildCharts(data: ExaminationStat[]): void {
    ...
    const dates = sorted.map(d => this.datePipe.transform(d.NGAY_TIEP_NHAN, 'dd/MM'));
    ...
    const commonOps = {
       ...
       tooltip: { trigger: 'axis', backgroundColor: this.palette.bgCard, borderColor: this.palette.gray200, textStyle: { color: this.palette.textPrimary } },
       grid: { left: '2%', right: '3%', bottom: '10%', top: '10%', containLabel: true }
    };

    this.trendChartOptions = {
      ...commonOps,
      legend: { bottom: 0, textStyle: { color: this.palette.textSecondary } },
      xAxis: { type: 'category', boundaryGap: false, data: dates, axisLine: {show: false}, axisLabel: {color: this.palette.textPrimary} },
      yAxis: { type: 'value', splitLine: { lineStyle: { type: 'dashed', color: this.palette.gray200 } } },
      series: [
        { name: 'Tổng Tiếp Nhận', type: 'line', smooth: true, showSymbol: false, data: sorted.map(d => d.TONG_LUOT_TIEP_NHAN), itemStyle: { color: c.total } },
        { name: 'Khám Bệnh (CK)', type: 'line', smooth: true, showSymbol: false, data: sorted.map(d => d.LUOT_KHAM_CK), itemStyle: { color: c.ck } },
        { name: 'Cấp Cứu', type: 'line', smooth: true, showSymbol: false, data: sorted.map(d => d.LUOT_CC), itemStyle: { color: c.cc } },
        { name: 'Nội Trú', type: 'line', smooth: true, showSymbol: false, data: sorted.map(d => d.LUOT_NT), itemStyle: { color: c.nt } },
        { name: 'ĐT Ngoại Trú', type: 'line', smooth: true, showSymbol: false, data: sorted.map(d => d.LUOT_DNT), itemStyle: { color: c.dnt } }
      ]
    };

    const pieTotals = sorted.reduce((a,b) => ({ bhyt: a.bhyt + (b.BHYT||0), vp: a.vp + (b.VIEN_PHI||0) }), {bhyt:0, vp:0});
    this.typeChartOptions = {
      ...commonOps,
      tooltip: { trigger: 'item', formatter: '{b}: {c} ({d}%)', backgroundColor: this.palette.bgCard, borderColor: this.palette.gray200, textStyle: { color: this.palette.textPrimary } },
      legend: { bottom: 0, textStyle: { color: this.palette.textSecondary } },
      series: [{
        type: 'pie', radius: ['45%', '70%'], center: ['50%', '45%'],
        itemStyle: { borderRadius: 5, borderColor: this.palette.bgCard, borderWidth: 2 },
        label: { show: false, position: 'center' },
        emphasis: { label: { show: true, fontSize: 18, fontWeight: 'bold', color: this.palette.textPrimary } },
        data: [
    ...
    this.admissionChartOptions = {
      ...commonOps,
      xAxis: { type: 'category', data: ['Khám CK', 'Cấp Cứu', 'Nội Trú', 'ĐT Ngoại Trú'], axisLabel: { color: this.palette.textPrimary } },
      yAxis: { type: 'value', splitLine: { lineStyle: { type: 'dashed', color: this.palette.gray200 } } },
      series: [{
  ...
  public trackByWidget(index: number, item: WidgetData): string { return item.id; }
  public onExport(): void { this.toastService.showInfo('Tính năng đang phát triển'); }
}