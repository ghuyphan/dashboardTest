...
import { ClsLevel3Stat } from '../../../shared/models/cls-stat.model';
...
@Component({
  selector: 'app-cls-level3-report',
  standalone: true,
  imports: [
    CommonModule,
    ChartCardComponent,
  ...
  templateUrl: './cls-level3-report.component.html',
  styleUrl: './cls-level3-report.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class ClsLevel3ReportComponent implements OnInit {
  ...
  public loadData(): void {
    ...
    this.reportService.getClsLevel3Report(this.fromDate, this.toDate)
      .pipe(
        finalize(() => {
  ...
  private processData(data: ClsLevel3Stat[]): void {
...
private buildCharts(
    ...
    // --- 2. ROOM CHART (Updated with DataZoom) ---
    // Sort Ascending because ECharts Y-Axis categories start from bottom
    const roomData = Array.from(roomMap.entries()).sort((a, b) => a[1] - b[1]);

    // Configuration for scrolling
    const BARS_TO_SHOW = 8; // Number of bars visible at once
    const totalBars = roomData.length;
    const enableZoom = totalBars > BARS_TO_SHOW;
    
    // Calculate range to show the TOP items (end of array) by default
    ...
    this.roomChartOptions = {
      ...
      grid: { 
        left: '3%', 
        // Increase right padding if scrollbar is visible to prevent overlap
      ...
      // ADDED DATA ZOOM HERE
      dataZoom: enableZoom ? [
        {
          type: 'slider',
          yAxisIndex: 0,
          width: 12,
          right: '2%', // Position slider on the far right
          top: '5%',
          bottom: '5%',
          startValue: startValue,
          endValue: endValue,
          fillerColor: this.palette.secondary + '40', // Semi-transparent brand color
          borderColor: 'transparent',
          handleSize: '0%', // Hide the drag handles for a cleaner "scrollbar" look
          showDetail: false, // Hide the text detail on the side
          brushSelect: false
        },
        {
          type: 'inside', // Allow scrolling with mouse wheel
          yAxisIndex: 0,
          startValue: startValue,
          endValue: endValue,
          zoomOnMouseWheel: false, // Disable zooming (scaling bar height)
          moveOnMouseWheel: true   // Enable scrolling up/down
        }
      ] : undefined,
      series: [{
        name: 'Số Lượng',
  ...
  public onExport(): void {
    ...
    setTimeout(() => {
        ...
        this.excelService.exportToExcel(
            this.rawData,
            `BaoCao_CLS_Tang3_${this.fromDate}_${this.toDate}`,
            columns
        );
